...
def audit_training(config):
    """Проверяет разделение групп и заново восстанавливает обучающие метки по ID."""
    work = config["work_dir"]
    queries = {fold: pl.read_parquet(work / f"validation/{fold}_queries.parquet")
               for fold in ["train", "dev", "holdout"]}
    # Совпадение группы в разных частях позволило бы запомнить формулировку запроса.
    ...
    # Номер запроса из таблицы признаков должен ссылаться на выбранный train-контекст.
    ...
    # Восстанавливаем строковые ID независимо от кода, который создавал признаки.
    ...
    # Метка равна единице только для точной пары контекст-объявление из train.
    ...
    # Служебные ID и метка могут храниться в таблице, но не входят в список признаков.
    ...
    # Для этого аудита не требуется открывать положительные пары dev или holdout.
...
def verify(config):
    """Пересчитывает Recall по сохранённым ID и проверяет итоговый CSV."""
    # Проверочная разметка читается только для неизменного зафиксированного эксперимента.
    check_frozen(config)
    report = {}
    corpus = set(pl.read_parquet(config["work_dir"] / "validation/corpus_manifest.parquet")["item_id"])
    # Проверяем обе части независимо от расчёта метрики в модуле обучения.
    for fold in ["dev", "holdout"]:
        ...
        # Пропущенные или повторные запросы исказили бы среднюю метрику.
        ...
        values = []
        # Здесь используются множества реальных ID, без внутренних doc_id и label.
        ...
        # Сверяем именно macro Recall, где каждый запрос имеет одинаковый вес.
    ...
    # Финальный файл проверяется относительно benchmark, а не расширенного корпуса.
    ...
    # Версии библиотек помогают воспроизвести проверку на другом компьютере.
