...
def audit_training(config):
    """Проверяет состав истории, список признаков и соответствие меток исходным парам."""
    work = config.get('quality_dir', config['work_dir'] / 'quality')
    base = config['work_dir'] / 'validation'
    # Проверочные группы не должны участвовать в обучающей истории.
    ...
    # ID и метки нужны для сопоставления строк, но не для обучения признаков.
    ...
    # Заново связываем номера строк с исходными ID, независимо от подготовки признаков.
    ...
    # Единица допустима только у пары, присутствующей в исходной разметке.
    ...
    # Сохраняем итог проверки; исключение своей группы выполняется в QueryHistory.search.
