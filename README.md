...
Проверенная версия — Python 3.12.10. Команды выполняются из корня репозитория.
...
`requirements-notebook.txt` фиксирует также Jupyter и его зависимости.
...
**Полный пересчёт с нуля проверен:** в новом клоне и изолированном Python-окружении
заново построены индексы и признаки, обучена модель и сформирован CSV.
Разбиение, обучающие признаки, функция модели и метрики совпали;
`answer.csv` совпал побайтово. Условия и границы проверки описаны
в [docs/reproduction.md](docs/reproduction.md).

Проверено на Windows с Python 3.12.10. Все 18 кодовых ячеек полного расчёта
выполнены последовательно через Python; отдельно проверены 18 ячеек режима
просмотра через Jupyter в том же окружении. Сохранённый основной notebook
