...
`answer.csv` соответствует предоставленному файлу `reply_2589047_4.txt`:
UTF-8, разделитель запятая, окончания строк CRLF, две колонки `query_id` и `answer`.
В нём 2 452 запроса и по 50 уникальных item_id. Проверка содержимого,
побайтового совпадения и контрольная сумма:
...
Символьный TF-IDF и обучаемые связи между локациями рассматриваются отдельно
в `experiments/` и анализе ошибок. Они не используются для приложенного `answer.csv`.

Проверены формат CSV и все идентификаторы. Полный пересчёт benchmark с этой
моделью проверен в новом окружении. После согласования переводов строк с
отправленным файлом повторены предсказания и 27 тестов: CSV совпал побайтово.
Повторное обучение в чистом окружении не выполнялось.
Подробности: [проверка воспроизводимости](docs/reproduction.md).
