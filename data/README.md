...
Для пересчёта в `check.ipynb` и полного запуска `solution.ipynb` с
`rebuild = True` нужны три исходных файла из задания:
...
Для просмотра сохранённых результатов в `solution.ipynb` с `rebuild = False`
исходные Parquet не нужны.
